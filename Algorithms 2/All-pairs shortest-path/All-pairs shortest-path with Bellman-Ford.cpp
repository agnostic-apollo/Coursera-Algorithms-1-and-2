...
void bellman_ford(vector< nodes > &graph ,int total_vertices,int total_edges , int source)
{
    ...
    vector< vector< int > > dist(total_vertices+1,vector< int > (total_vertices+1,10000000));
    list< int > changed_vertices(0);
    unordered_set< int > changed_vertices_set;



    int total_changed_vertices,iterations;


    //run bellman-ford early exit on vertex 1 as source to detect negative cycles
    n=1;
    dist[n][n]=0;
    ...
    while (!changed_vertices.empty())
    {
        ...
        if(iterations==total_vertices-1)
            break;
        ...
        for(r=0;r<total_changed_vertices;r++)
        {
           ...
           for(j=0;j!=graph[i].edge_list.size();j++)
            {
                if( dist[n][i] + graph[i].length_list[j] < dist[n][graph[i].edge_list[j]] )
                {
                    dist[n][graph[i].edge_list[j]] = dist[n][i] + graph[i].length_list[j];
    ...
    //run bellman-ford early exit on all vertices starting from 2 as source for APSP
    for(n=2;n!=graph.size();n++)
    {
    ...
    for(n=1;n!=dist.size();n++)
        for(i=1;i!=dist[n].size();i++)
            if(dist[n][i]<shortest)
                shortest=dist[n][i];
